...
impl RuntimeSignature {
    pub fn llvm_decl(self, symbol: &str) -> String {
        match self {
            ...
            Self::VoidEchoValueEchoValue => {
                format!("declare void @{symbol}(%EchoValue, %EchoValue)")
            }
...
mod tests {
    ...
    #[test]
    fn runtime_symbols_render_known_declarations() {
        assert_eq!(
            CoreRuntimeSymbol::Write.llvm_decl(),
            "declare void @echo_write(ptr, i64)"
        );
        assert_eq!(
            CoreRuntimeSymbol::ValueAdd.llvm_decl(),
            "declare %EchoValue @echo_value_add(%EchoValue, %EchoValue)"
        );
    }

    #[test]
    fn php_and_std_lookup_tables_find_known_entries() {
        let strlen = php_builtin("strlen").expect("strlen builtin");
        assert_eq!(strlen.symbol, "echo_php_strlen");

        let net_listen = std_intrinsic("net.listen").expect("net.listen intrinsic");
        assert_eq!(net_listen.symbol, "echo_std_net_listen");
    }

    #[test]
    fn runtime_signatures_render_llvm_declarations() {
        assert_eq!(
            RuntimeSignature::EchoValueEchoValueEchoValue.llvm_decl("echo_value_add"),
            "declare %EchoValue @echo_value_add(%EchoValue, %EchoValue)"
        );
        assert_eq!(
            RuntimeSignature::VoidPtrI64.llvm_decl("echo_write"),
            "declare void @echo_write(ptr, i64)"
        );
    }
}
