    ...
    fn render_mir_function_call_expr(
        ...
        match builtin.codegen {
            ...
            BuiltinCodegen::Clearstatcache => {
                ...
                let clear_realpath_cache = if let Some(arg) = call.args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 1, i64 0 }".to_string()
                };
                let filename = if let Some(arg) = call.args.get(1) {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
            ...
            BuiltinCodegen::Header => {
                ...
                let header = self.render_mir_expr_as_echo_value(body, &call.args[0])?;
                let replace = if let Some(arg) = call.args.get(1) {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 1, i64 1 }".to_string()
                };
                let response_code = if let Some(arg) = call.args.get(2) {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 1, i64 0 }".to_string()
                };
                ...
                body.push_str(&format!(
                    "  call void @{}({header}, {replace}, {response_code})\n",
                    builtin.symbol
                ));
                Ok(RuntimeValue::EchoValue("{ i32 0, i64 0 }".to_string()))
            }
            BuiltinCodegen::HeaderRemove => {
                if call.args.len() > 1 {
                    return Err(Diagnostic::new(
                        format!(
                            "unsupported argument count for builtin `{}` in LLVM codegen",
                ...
                let name = if let Some(arg) = call.args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };

                body.push_str(&format!("  call void @{}({name})\n", builtin.symbol));
                Ok(RuntimeValue::EchoValue("{ i32 0, i64 0 }".to_string()))
            }
            BuiltinCodegen::Gettimeofday => {
                if call.args.len() > 1 {
                    return Err(Diagnostic::new(
                        format!(
                            "unsupported argument count for builtin `{}` in LLVM codegen",
                ...
                let as_float = if let Some(arg) = call.args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                ...
                body.push_str(&format!(
                    "  %runtime_call_{call_id} = call %EchoValue @{}({as_float})\n",
                    builtin.symbol
                ));
                Ok(RuntimeValue::EchoValue(format!("%runtime_call_{call_id}")))
            }
            BuiltinCodegen::Hrtime => {
                if call.args.len() > 1 {
                    return Err(Diagnostic::new(
                        format!(
                            "unsupported argument count for builtin `{}` in LLVM codegen",
                ...
                let as_number = if let Some(arg) = call.args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                ...
                body.push_str(&format!(
                    "  %runtime_call_{call_id} = call %EchoValue @{}({as_number})\n",
                    builtin.symbol
                ));
                Ok(RuntimeValue::EchoValue(format!("%runtime_call_{call_id}")))
            }
            BuiltinCodegen::HttpResponseCode => {
                if call.args.len() > 1 {
                    return Err(Diagnostic::new(
                        format!(
                            "unsupported argument count for builtin `{}` in LLVM codegen",
                ...
                let response_code = if let Some(arg) = call.args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                ...
                body.push_str(&format!(
                    "  %runtime_call_{call_id} = call %EchoValue @{}({response_code})\n",
                    builtin.symbol
                ));
                Ok(RuntimeValue::EchoValue(format!("%runtime_call_{call_id}")))
            }
            ...
            BuiltinCodegen::ValueOptionalNullExpression => {
                if call.args.len() > 1 {
                    return Err(Diagnostic::new(
                        format!(
                            "unsupported argument count for builtin `{}` in LLVM codegen",
            ...
            BuiltinCodegen::PhpVersion => {
                ...
                let extension = match call.args.first() {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 0, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
            ...
            BuiltinCodegen::ValueUnaryOptionalNullExpression => {
                ...
                let optional = match call.args.get(1) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 0, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({value}, {optional})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::IniGetAll => {
                ...
                let extension = if let Some(arg) = call.args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                let details = if let Some(arg) = call.args.get(1) {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 1, i64 1 }".to_string()
                };
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({extension}, {details})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::Pathinfo => {
                ...
                let path = self.render_mir_expr_as_echo_value(body, &call.args[0])?;
                let flags = match call.args.get(1) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 0, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({path}, {flags})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::ValueBinaryOptionalNullExpression => {
                ...
                let optional = match call.args.get(2) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 0, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({left}, {right}, {optional})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::ValueBinaryOptionalIntOneExpression => {
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({value}, {other}, {step})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::Round => {
                ...
                let value = self.render_mir_expr_as_echo_value(body, &call.args[0])?;
                let precision = match call.args.get(1) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 2, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({value}, {precision})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::Wordwrap => {
                ...
                let string = self.render_mir_expr_as_echo_value(body, &call.args[0])?;
                let width = match call.args.get(1) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 2, i64 75 }".to_string(),
                };
                let break_string = match call.args.get(2) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => self.runtime_value_as_echo_value(
                        body,
                        RuntimeValue::StaticString("\n".to_string()),
                ...
                let cut_long_words = match call.args.get(3) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 1, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({string}, {width}, {break_string}, {cut_long_words})\n",
                    builtin.symbol
                ));
            ...
            BuiltinCodegen::SubstrReplace => {
                ...
                let string = self.render_mir_expr_as_echo_value(body, &call.args[0])?;
                let replacement = self.render_mir_expr_as_echo_value(body, &call.args[1])?;
                ...
                body.push_str(&format!(
                    "  {name} = call %EchoValue @{}({string}, {replacement}, {offset}, {length})\n",
                    builtin.symbol
                ));
