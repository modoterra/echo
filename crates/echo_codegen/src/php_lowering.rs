    ...
    pub(super) fn mir_php_builtin_call(
        ...
        match builtin.codegen {
            ...
            BuiltinCodegen::Header => {
                ...
                let header = self.render_mir_expr_as_echo_value(body, &args[0])?;
                let replace = if let Some(arg) = args.get(1) {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 1, i64 1 }".to_string()
                };
                let response_code = if let Some(arg) = args.get(2) {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 1, i64 0 }".to_string()
                };
            ...
            BuiltinCodegen::Gettimeofday => {
                ...
                let as_float = if let Some(arg) = args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                body.push_str(&format!(
                    "  call %EchoValue @{}({as_float})\n",
                    builtin.symbol
                ));
            }
            BuiltinCodegen::Hrtime => {
                ...
                let as_number = if let Some(arg) = args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                body.push_str(&format!(
                    "  call %EchoValue @{}({as_number})\n",
                    builtin.symbol
                ));
            }
            BuiltinCodegen::HttpResponseCode => {
                ...
                let response_code = if let Some(arg) = args.first() {
                    self.render_mir_expr_as_echo_value(body, arg)?
                } else {
                    "%EchoValue { i32 0, i64 0 }".to_string()
                };
                body.push_str(&format!(
                    "  call %EchoValue @{}({response_code})\n",
                    builtin.symbol
                ));
            }
            ...
            BuiltinCodegen::ValueOptionalNullExpression => {
                ...
                body.push_str(&format!(
                    "  %runtime_call_{call_id} = call %EchoValue @{}({value})\n",
                    builtin.symbol
                ));
            }
            ...
            BuiltinCodegen::ValueUnaryOptionalNullExpression => {
                ...
                let value = self.render_mir_expr_as_echo_value(body, &args[0])?;
                let optional = match args.get(1) {
                    Some(expr) => self.render_mir_expr_as_echo_value(body, expr)?,
                    None => "%EchoValue { i32 0, i64 0 }".to_string(),
                };
                let call_id = self.next_call_id;
                self.next_call_id += 1;
                body.push_str(&format!(
                    "  %runtime_call_{call_id} = call %EchoValue @{}({value}, {optional})\n",
                    builtin.symbol
                ));
            }
