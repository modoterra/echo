...
Use the default array form when legacy code expects named `sec` and `usec` fields, or pass `true` for a float timestamp. New Echo code should prefer `std.time` APIs for elapsed-duration measurement.

`hrtime()` is useful when compatibility code needs a high-resolution timestamp shape without formatting a wall-clock date:
...
Use the default array form when ported PHP code expects `[seconds, nanoseconds]`, or pass `true` when a single nanosecond count is easier to store. New Echo code should prefer `std.time` timers for elapsed-duration measurement.

`clearstatcache()` is useful when compatibility code deliberately refreshes filesystem metadata after creating or replacing files:
...
Echo currently treats `clearstatcache()` as a no-op because filesystem helpers read local metadata directly and Echo does not model PHP's stat or realpath cache yet.
...
`connection_aborted()` and `connection_status()` are useful when compatibility code checks whether a web client disconnected before doing more response work:
...
Echo currently returns `0` for both functions because compiled programs use CLI-style execution and Echo does not model an abortable client connection yet.

`ignore_user_abort()` is useful when compatibility code records whether work should continue after a web client disconnects:
...
Echo tracks the setting as process-local state so later calls can observe it. Echo does not model an abortable client connection yet, so the setting does not affect process execution.
...
`http_response_code()` is useful when compatibility code records or inspects the response status before emitting a body:
...
Echo tracks the response code as process-local state so CLI-style compatibility code can observe the same value later. Echo does not model an HTTP header layer yet, so setting the code does not send a status line.
