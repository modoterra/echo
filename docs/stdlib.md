...
`cli_set_process_title()` and `cli_get_process_title()` are useful when compatibility code labels a long-running CLI worker:
...
Echo stores the title in process-local runtime state so compatibility code can read it back. Host OS process-title mutation is deferred.
