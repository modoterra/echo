...
| `foreach` statements | partial | Parser/AST support covers `foreach (iterable as $value)` and `foreach (iterable as $key => $value)` with braced and alternate `foreach (...): ... endforeach;` bodies. Destructuring targets, by-reference values, iterable diagnostics, and runtime iteration are deferred. Source: https://www.php.net/manual/en/control-structures.foreach.php |
