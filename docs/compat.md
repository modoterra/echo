...
| `convert_uudecode` | implemented | Decodes PHP uuencoded line data and returns `false` for invalid input. Source: https://www.php.net/manual/en/function.convert-uudecode.php |
...
| `getrusage` | partial | Returns the standard resource-usage array shape for the current process; Linux builds populate user/system CPU time and peak resident memory from `/proc`, while child-process mode is deferred. Source: https://www.php.net/manual/en/function.getrusage.php |
...
| `http_clear_last_response_headers` | partial | Clears no stored HTTP wrapper response state because Echo does not yet populate last response headers from URL wrappers. Source: https://www.php.net/manual/en/function.http-clear-last-response-headers.php |
| `http_get_last_response_headers` | partial | Returns `null` when no HTTP wrapper response headers have been recorded; URL-wrapper header capture is deferred. Source: https://www.php.net/manual/en/function.http-get-last-response-headers.php |
...
| `output_add_rewrite_var` | partial | Accepts string name/value pairs and returns `true`; storing rewrite variables, starting the `URL-Rewriter` handler, and rewriting output are deferred. Source: https://www.php.net/manual/en/function.output-add-rewrite-var.php |
...
| `passthru` | partial | Executes a command through the Linux/macOS shell, writes stdout to runtime output, and returns `null` on success; by-reference `result_code`, stderr handling, disabled-function policy, `ValueError`/warning emission, and interactive process behavior are deferred. Source: https://www.php.net/manual/en/function.passthru.php |
...
| `realpath_cache_get` | implemented | Returns an empty array because Echo does not model PHP's realpath cache. Source: https://www.php.net/manual/en/function.realpath-cache-get.php |
...
| `stream_get_line` | partial | Reads from Echo local file streams up to an explicit length, delimiter, or EOF and omits the delimiter when consumed; optional argument lowering, socket edge cases, and PHP warning/value diagnostics are deferred. Source: https://www.php.net/manual/en/function.stream-get-line.php |
...
| `stream_set_timeout` | partial | Returns PHP's false status for regular local file streams; socket timeout configuration, timeout state propagation, optional argument lowering, and PHP warning/type diagnostics are deferred. Source: https://www.php.net/manual/en/function.stream-set-timeout.php |
