...
| short echo tags | partial | Parser/AST support covers standalone `<?= expr ?>` short echo tags and lowers them to `EchoStmt`. Mixed inline HTML/PHP template interleaving around short echo tags is deferred. Source: https://www.php.net/manual/en/language.basic-syntax.phptags.php |
