...
| PHP tags | partial | Parser/AST support covers `<?php` open tags, final `?>` close tags as statement terminators, and standalone `<?= expr ?>` short echo tags lowered to `EchoStmt`. Mixed inline HTML/PHP template interleaving around tags is deferred. Sources: https://www.php.net/manual/en/language.basic-syntax.phptags.php and https://www.php.net/manual/en/language.basic-syntax.instruction-separation.php |
