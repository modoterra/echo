...
- PHP compatibility fixtures: 200 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 226.
- Remaining Core + standard functions in inventory: 381.
...
coverage alone is `226 / 607`, or about 37%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~32% | Core + standard function coverage is 226/607, with deeper object/error/extension behavior still missing. |
...
- `[ ]` Raise implemented baseline count beyond 226/607.
