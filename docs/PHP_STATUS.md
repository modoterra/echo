...
- PHP compatibility fixtures: 221 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 254.
- Remaining Core + standard functions in inventory: 353.
...
coverage alone is `254 / 607`, or about 42%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~34% | Core + standard function coverage is 254/607, with deeper object/error/extension behavior still missing. |
