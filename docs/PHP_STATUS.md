...
- PHP compatibility fixtures: 208 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 240.
- Remaining Core + standard functions in inventory: 367.
...
coverage alone is `240 / 607`, or about 40%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~33% | Core + standard function coverage is 240/607, with deeper object/error/extension behavior still missing. |
