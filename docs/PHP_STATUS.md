...
- PHP compatibility fixtures: 219 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 252.
- Remaining Core + standard functions in inventory: 355.
...
coverage alone is `252 / 607`, or about 42%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~34% | Core + standard function coverage is 252/607, with deeper object/error/extension behavior still missing. |
