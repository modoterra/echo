...
- PHP compatibility fixtures: 349 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 398.
- Remaining Core + standard functions in inventory: 209.
...
coverage alone is `398 / 607`, or about 66%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~38% | Core + standard function coverage is 398/607, with deeper object/error/extension behavior still missing. |
...
- PHP string literal escape handling is incomplete for hexadecimal byte escapes
  such as `"\xEB"`. This blocks byte-oriented fixtures from using common PHP
...
- PHP URL parsing currently covers common absolute URLs only. Component
  constants, relative and malformed URL quirks, empty query/fragment
  distinctions, and control-character substitution remain family-level gaps for
  `parse_url()` and future URI compatibility.
- Numeric and byte sequence helpers currently cover integer and one-byte string
  paths first. Float ranges, PHP `ValueError` surfaces, and coercion warnings
  remain common gaps for `range()`-style sequence generation.
- Directory listing currently covers single-call local directory snapshots.
  Persistent directory handles, sorting constants, stream contexts, URL
  wrappers, and warning emission remain common gaps for `scandir()`/`opendir()`
  style APIs.
- Filesystem capacity helpers use the local Linux/macOS `statvfs` view only.
  Remote wrappers, `open_basedir`, warning emission, and PHP's unspecified
  behavior for file paths passed where directories are expected remain gaps for
  `disk_free_space()`/`disk_total_space()` style APIs.
...
- Include-path resolution currently canonicalizes existing local paths only.
  Actual `include_path` searching, custom wrappers, URL wrappers, cache quirks,
  and PHP warning/type diagnostics remain gaps for
  `stream_resolve_include_path()`.
- Stream lock support currently reports whether an Echo local file stream is
...
- Stream line reads currently cover local file stream bytes, explicit length,
  ...
  PHP `ValueError`/warning diagnostics remain gaps for `stream_get_line()`.
...
- Serialization currently covers scalar values and PHP arrays only. Object
  hooks, resources, references/cycles, Echo lists, exact float serialization
  parity, and `unserialize()` remain gaps for the serialization family.
- Debug dumping/export currently covers scalar and PHP array `var_dump()`,
  `var_export()`, and `print_r()` values. Objects, resources,
  references/cycles, variadic dumping, and exact recursive edge cases remain
  gaps for the debug rendering family.
...
- Shell execution currently covers simple `shell_exec()` stdout capture,
  `exec()` last-line stdout return values, `passthru()` direct stdout
  streaming, and `system()` stdout streaming with last-line return values
  through the Linux/macOS shell. By-reference output arrays/result codes, exit
  status reporting, stderr capture, `proc_*` handles, interactive streams,
  timeout/cancellation, and PHP warning emission remain gaps for the process
  execution family.
- PHP information output is compact and CLI-oriented. Full `phpinfo()` and
  ...
  compatibility.
- Version comparison supports PHP-standardized numeric and pre-release ordering
...
- Locale-aware collation is not modeled yet. `strcoll()` currently follows the
  C/POSIX locale byte-order baseline; `setlocale()`, locale categories, and
