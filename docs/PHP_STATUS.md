...
- PHP compatibility fixtures: 240 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 273.
- Remaining Core + standard functions in inventory: 334.
...
coverage alone is `273 / 607`, or about 45%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~34% | Core + standard function coverage is 273/607, with deeper object/error/extension behavior still missing. |
