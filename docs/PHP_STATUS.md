...
- PHP compatibility fixtures: 347 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 396.
- Remaining Core + standard functions in inventory: 211.
...
coverage alone is `396 / 607`, or about 65%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~38% | Core + standard function coverage is 396/607, with deeper object/error/extension behavior still missing. |
...
- Shell execution currently covers simple `shell_exec()` stdout capture,
  `passthru()` direct stdout streaming, and `system()` stdout streaming with
  last-line return values through the Linux/macOS shell.
  By-reference result codes, exit status reporting, stderr capture, `proc_*`
  handles, interactive streams, timeout/cancellation, and PHP warning emission
  remain gaps for the process execution family.
