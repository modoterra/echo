...
- PHP compatibility fixtures: 202 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 230.
- Remaining Core + standard functions in inventory: 377.
...
coverage alone is `230 / 607`, or about 38%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~32% | Core + standard function coverage is 230/607, with deeper object/error/extension behavior still missing. |
...
- `[ ]` Raise implemented baseline count beyond 230/607.
