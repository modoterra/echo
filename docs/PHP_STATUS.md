...
- PHP compatibility fixtures: 203 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 232.
- Remaining Core + standard functions in inventory: 375.
...
coverage alone is `232 / 607`, or about 38%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~32% | Core + standard function coverage is 232/607, with deeper object/error/extension behavior still missing. |
...
- `[ ]` Raise implemented baseline count beyond 232/607.
