...
- PHP compatibility fixtures: 198 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 216.
- Remaining Core + standard functions in inventory: 391.
...
coverage alone is `216 / 607`, or about 36%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~31% | Core + standard function coverage is 216/607, with deeper object/error/extension behavior still missing. |
...
- `[ ]` Raise implemented baseline count beyond 216/607.
