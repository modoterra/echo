...
- PHP compatibility fixtures: 210 `tests/php/*/program.php` files.
- Echo fixtures: 90 `tests/echo/*/program.echo` files.
- Core + standard PHP functions in inventory: 607.
- Implemented Core + standard functions in inventory: 242.
- Remaining Core + standard functions in inventory: 365.
...
coverage alone is `242 / 607`, or about 40%, for the Core + standard baseline,
...
| Runtime behavior and built-ins | 35% | ~33% | Core + standard function coverage is 242/607, with deeper object/error/extension behavior still missing. |
