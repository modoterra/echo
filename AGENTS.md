...
- Echo compiles a closed `CompilationGraph`. Static includes, package/std metadata, and source-level `compile { ... }` declarations admit graph members. `compile` entries resolve as follows: `"./relative"` is rooted at the declaring file's `__DIR__`, `"/absolute"` is a host filesystem path, and `"name/package"` loads the whole package. Do not add compiled/JIT behavior that searches arbitrary runtime filesystem paths or depends on Composer's generated autoload file after graph construction; dynamic includes outside the graph should become explicit diagnostics or runtime errors.
...
- Format commit descriptions as real multi-paragraph text. When using `git commit -m`, pass each paragraph as a separate `-m` argument or use an editor/template; do not embed escaped newline text such as `\n\n`. Before amending or pushing, inspect `git log -1 --pretty=fuller` when the body was assembled from shell arguments.
